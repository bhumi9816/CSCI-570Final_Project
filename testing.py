...
def mis_match_score(char1, char2, seq=['A', 'C', 'G', 'T'], alpha_matrix=[[0,110,48,94], [110, 0, 118, 48], [48, 118, 0, 110], [94, 48, 110, 0]]):
    i = seq.index(char1)
    j = seq.index(char2)
    #print('value is ', i, '....', j, alpha_matrix[i][j])
...
def match_character(str1, str2):
    if str1 == str1:
        return 0
    elif str1 == '-' or str2 == '-':
        return gap_penalty
    else:
        misMatch = align_score(str1, str2)
        return misMatch
...
def cost_generator(str1, str2):
    ...
    while i > 0 and j > 0:
        ...
        print("the character alignment is .... ", str2[j-1], '.....', str1[i-1])

        print("the score is ... ", match_score(str2[j-1], str1[i-1]))

        total_score = 0
    
        if curr_score == diagonal_score + match_score(str1[j-1], str2[i-1]):
            align_str1 += str1[j-1]
            align_str2 += str2[i-1]
            total_score += score[i-1][j-1]
            i -= 1
            j -= 1
            ...
            align_str1 += str1[j-1]
            align_str2 += '-'
            total_score += score[i][j-1]
            j -= 1

        elif curr_score == left_score + gap_penalty:
            align_str2 += str2[i-1]
            align_str1 += '-'
            total_score += score[i-1][j]
            i -= 1
